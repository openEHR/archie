...
public class SimpleMetaModelProvider implements MetaModelProvider {
     ...
     *
     * @param referenceModels the ReferenceModels to use for retrieving meta models or null
     * @param bmmRepository   the BmmRepository to use for retrieving BMM models or null
     * @throws IllegalArgumentException if both referenceModels and bmmRepository are null
     */
    ...
    public SimpleMetaModelProvider(ReferenceModels referenceModels, BmmRepository bmmRepository, AomProfiles aomProfiles) {
        ...
        this.aomProfiles = Objects.requireNonNull(aomProfiles, "aomProfiles must not be null");
    }
    ...
    public MetaModel getMetaModel(String rmPublisher, String rmPackage, String rmRelease) throws ModelNotFoundException {
        ...
        if (bmmRepository != null) {
            BmmValidationResult validationResult = bmmRepository.getModelByClosure(
                    BmmDefinitions.publisherQualifiedRmClosureName(rmPublisher, rmPackage) + "_" + rmRelease
            );
        ...
        if (modelInfoLookup == null && bmmModel == null) {
            throw new ModelNotFoundException(
                    String.format("model for %s.%s version %s not found", rmPublisher, rmPackage, rmRelease)
            );
        }
